...
! function(a, b, c) {
	...
	d.prototype = {
		init: function() {
			...
			"click" === this.getTrigger() ? this.$element.off("click").on("click", a.proxy(this.toggle, this)) : "hover" === this.getTrigger() && this.$element.off("mouseenter mouseleave click").on("mouseenter", a.proxy(this.mouseenterHandler, this)).on("mouseleave", a.proxy(this.mouseleaveHandler, this)), this._poped = !1, this._inited = !0, this._opened = !1, this._idSeed = k, this.options.backdrop && j.appendTo(this.options.$container).hide(), k++
		},
		destroy: function() {
		...
		hide: function(b) {
			if(this._opened) {
				b && (b.preventDefault(), b.stopPropagation()), this.xhr && (this.xhr.abort(), this.xhr = null);
				var c = a.Event("hide." + g);
				this.$element.trigger(c), this.$target && this.$target.removeClass("in").hide(), this.options.backdrop && j.hide(), this._opened = !1, this.$element.trigger("hidden." + g),this.options.onHide && this.options.onHide(b)
			}
		},
		toggle: function(a) {
		...
		show: function() {
			...
			if(this.options.multi || this.hideAll(), !this._opened) {
				if(!this.getCache() || !this._poped || "" === this.content) {
					if(this.content = "", this.setTitle(this.getTitle()), this.options.closeable || a.find(".close").off("click").remove(), this.isAsync()) return this.setContentASync(this.options.content), void this.displayContent();
					this.setContent(this.getContent()), a.show()
				}
				this.displayContent(), this.options.onShow && this.options.onShow(a), this.bindBodyEvents(), this.options.backdrop && j.show(), this._opened = !0
			}
		},
...
//			return;
			var b = this.getElementPosition(),
				...
				j = "bottom",
				k = a.Event("show." + g);
			this.$element.trigger(k), "auto" !== this.options.width && d.width(this.options.width), "auto" !== this.options.height && e.height(this.options.height), this.options.arrow || d.find(".arrow").remove(), d.detach().css({
				top: -2e3,
				left: -2e3,
				display: "block"
			}), this.getAnimation() && d.addClass(this.getAnimation()), d.appendTo(this.options.$container), h = d[0].offsetWidth, i = d[0].offsetHeight, j = this.getPlacement(b), this.initTargetEvents();
			...
			if(this.options.style && this.$target.addClass(f + "-" + this.options.style), this.options.padding || (e.css("height", e.outerHeight()), this.$target.addClass("webui-no-padding")), this.options.arrow || this.$target.css({
					margin: 0
				}), this.options.arrow) {
				var n = this.$target.find(".arrow");
				n.removeAttr("style"), l.arrowOffset && n.css(l.arrowOffset)
			}
			this._poped = !0, this.$element.trigger("shown." + g)
		},
		...
		getConstrains: function() {
			var a = this.$element.attr("data-contrains");
			return "undefined" != typeof a ? a : this.options.constrains
		},
		...
		setContentASync: function(b) {
			...
			this.xhr = a.ajax({
				...
				success: function(d) {
					...
					e.removeAttr("style"), c.displayContent(), c.options.async.success && c.options.async.success(c, d), this.xhr = null
				}
			})
		},
		...
		initTargetEvents: function() {
			"hover" === this.getTrigger() && this.$target.off("mouseenter mouseleave").on("mouseenter", a.proxy(this.mouseenterHandler, this)).on("mouseleave", a.proxy(this.mouseleaveHandler, this)), this.$target.find(".close").off("click").on("click", a.proxy(this.hide, this)), this.$target.off("click.webui-popover").on("click.webui-popover", a.proxy(this.targetClickHandler, this))
		},
		getPlacement: function(a) {
			...
			if(b = "function" == typeof this.options.placement ? this.options.placement.call(this, this.getTarget()[0], this.$element[0]) : this.$element.data("placement") || this.options.placement, "auto" === b) {
				var l = "horizontal" === this.getConstrains(),
					m = "vertical" === this.getConstrains();
				b = f / 3 > j ? g / 3 > k ? l ? "right-bottom" : "bottom-right" : 2 * g / 3 > k ? m ? g / 2 >= k ? "bottom-right" : "top-right" : "right" : l ? "right-top" : "top-right" : 2 * f / 3 > j ? g / 3 > k ? l ? f / 2 >= j ? "right-bottom" : "left-bottom" : "bottom" : 2 * g / 3 > k ? l ? f / 2 >= j ? "right" : "left" : g / 2 >= k ? "bottom" : "top" : l ? f / 2 >= j ? "right-top" : "left-top" : "top" : g / 3 > k ? l ? "left-bottom" : "bottom-left" : 2 * g / 3 > k ? m ? g / 2 >= k ? "bottom-left" : "top-left" : "left" : l ? "left-top" : "top-left"
			} else "auto-top" === b ? b = f / 3 > j ? "top-right" : 2 * g / 3 > j ? "top" : "top-left" : "auto-bottom" === b ? b = f / 3 > j ? "bottom-right" : 2 * g / 3 > j ? "bottom" : "bottom-left" : "auto-left" === b ? b = g / 3 > k ? "left-top" : 2 * g / 3 > k ? "left" : "left-bottom" : "auto-right" === b && (b = g / 3 > k ? "right-top" : 2 * g / 3 > k ? "right" : "right-bottom");
			return b
		},
		getElementPosition: function() {
			return a.extend({}, this.$element.position(), {
			...
			switch(b) {
				...
				case "top-right":
					h = {
						...
						left: Math.min(f, c) / 2 + k
					};
					break;
				case "top-left":
					h = {
						top: e.top - d,
						left: e.left - c + e.width + k
					}, i = {
						left: c - Math.min(f, c) / 2 - k
					};
					break;
				case "bottom-right":
					h = {
				...
				case "right-top":
					h = {
						top: e.top - d + e.height + l,
						left: e.left + e.width
					}, i = {
						top: d - Math.min(g, d) / 2 - l
					};
					break;
				case "right-bottom":
					h = {
