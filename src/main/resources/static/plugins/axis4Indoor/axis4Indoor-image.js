...
(function($) {
	$.axis4Indoor = function(params) {
		...
		that.doLocation = function(data) {
            pushCurrentData(data);
            locating(data);
		};
		
		function pushCurrentData(data) {
            var upflag=false;
            for(var i=0;i<that.currentData.length;i++){
                if(that.currentData[i].tagId==data.tagId){
                    that.currentData[i].x=data.x;
                    that.currentData[i].y=data.y;
                    that.currentData[i].battery=data.battery;
                    that.currentData[i].updateDate=data.updateDate;
                    upflag=true;
                    break;
                }
			}
		...
		function locating(n) {
			...
			if($coursor.length == 0) {
				params.$container.append('<span id="cursor_' + n.tagId + '" class="cursor '+(!hideAll?"":"hidden")+'"></span>');
				...
				$coursor.webuiPopover($.extend(popSettings, {
					...
					content: html.join(""),
					onShow: function() {
						$coursor.attr("data-popShow", 1);
					},
					onHide: function() {
						$coursor.attr("data-popShow", 0);
					}
				}));
            ...
            if($coursor.attr("data-popShow")==1){
		...
		that.resize = function() {
			...
			params.areaWidth = params.$container.height() * currentScale;
			...
			if(params.axisGrid) {
				...
				var tableHtml = [];
				//默认每1²米一个格子
				for(var i = 0; i < params.areaWidth / 1000; i++) {
					tableHtml.push('<tr>');
					for(j = 0; j < params.areaLength / 1000; j++) {
			...
			$.each(that.currentData,function (i,n) {
				that.doLocation(n);
			})
		};
		...
		that.openInfoWindow = function(tagId) {
			var $dom=$("#cursor_" + tagId);
			if($dom.length>0){
                $dom.webuiPopover('show');
                that.centerTo(tagId);
			}
		}
		/**
		...
		that.closeAllInfoWin=function(){
			$("span[data-popShow='1']").webuiPopover('hide');
            $("span[data-popShow='1']").attr("data-popShow",0);
		}


		//显示需要显示的定位点
        that.displayPoints=function (tagIds) {
            that.closeAllInfoWin();
            $("#axisContent .cursor").addClass("hidden");
			if(tagIds.length>0){
                hideAll=false;
                $.each(tagIds,function (i,n) {
                    $("#cursor_" + n).removeClass("hidden");
                });
			}else{
                hideAll=true;
			}
